...
<div class="bg-dark text-white py-4">
    <div class="container d-flex justify-content-between align-items-center">
        <div>
            <h1 class="fw-bold mb-0 text-start"><i class="bi bi-plus-circle me-2 text-warning"></i>Create New Product</h1>
            <small class="opacity-75">Fill in the details below to add a new product to the store.</small>
        </div>
        <a href="{{ route('admin.products.index') }}" class="btn btn-outline-light rounded-pill btn-sm px-4">
        ...
        <div class="row g-4">
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <div class="mt-1">
                                <small class="text-muted">Slug preview: <code id="slug-preview" class="text-primary">/shop/your-product-name</code></small>
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category_id" id="category_id" class="form-select bg-light border-0 py-3 @error('category_id') is-invalid @enderror" required>
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="description" id="description" rows="6"
                                ...
                                placeholder="Describe the product in detail — features, materials, specifications..."
                                maxlength="2000"
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mt-1">
                                    <small id="stock-status" class="text-muted">Enter a stock quantity above.</small>
                                </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        {{-- Image Preview Box --}}
                        <div id="image-preview-box" class="bg-light rounded-3 d-flex align-items-center justify-content-center mb-3"
                            ...
                            <div id="preview-placeholder" class="text-center text-muted">
                                ...
                                <small style="font-size: 0.7rem;">JPG, PNG, GIF — Max 2MB</small>
                            </div>
                            <img id="image-preview" src="" class="img-fluid rounded-3 d-none" style="max-height: 220px; object-fit: cover; width: 100%;" alt="Product Preview">
                        </div>
                        <input type="file" name="image" id="image" class="d-none" accept="image/*"
                            onchange="previewImage(event)" @error('image') style="border: 1px solid red;" @enderror>
                        <button type="button" class="btn btn-outline-secondary w-100 rounded-pill btn-sm" onclick="document.getElementById('image').click()">
                            <i class="bi bi-upload me-1"></i>Choose Image
                        </button>
                        @error('image') <small class="text-danger d-block mt-1">{{ $message }}</small> @enderror
                        <small class="text-muted d-block mt-2 text-center">Recommended: 800×800px or larger.</small>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center py-3 border-bottom">
                            ...
                            <div class="form-check form-switch m-0">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} style="width: 2.5rem; height: 1.25rem;">
                            </div>
                        </div>
                        <div class="d-flex justify-content-between align-items-center py-3">
                            ...
                            <div class="form-check form-switch m-0">
                                <input class="form-check-input" type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} style="width: 2.5rem; height: 1.25rem;">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <a href="{{ route('admin.products.index') }}" class="btn btn-link w-100 text-muted mt-2">Cancel</a>
            </div>
        </div>
    </form>
</div>
...
function updateStockStatus(val) {
    ...
    if (isNaN(qty) || val === '') { el.textContent = 'Enter a stock quantity above.'; el.className = 'text-muted'; return; }
    if (qty === 0) { el.innerHTML = '<i class="bi bi-x-circle me-1"></i>Out of Stock'; el.className = 'text-danger fw-semibold'; }
    else if (qty < 5) { el.innerHTML = '<i class="bi bi-exclamation-triangle me-1"></i>Low Stock Warning'; el.className = 'text-warning fw-semibold'; }
    else { el.innerHTML = '<i class="bi bi-check-circle me-1"></i>In Stock'; el.className = 'text-success fw-semibold'; }
}
